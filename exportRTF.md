## Export to RTF  <a id='export-rtf'></a>

_DOTE_ supports `Export to RTF`, since all word processors on different platforms can read lightly formatted RTF files. We also support `Export to SRT` because most desktop media players can play simple subtitles from such a file and overlay them on the video.
...
4. Choose your options from the check boxes, drop-down list and variables.
5. Select `EXPORT`.
6. Select a location and filename for the RTF export.
7. After a short pause, _DOTE_ will return to display the transcript.
8. You can find the RTF file in the folder you selected.
