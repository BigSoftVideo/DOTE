...
_DOTE_ is able to import transcripts prepared by most Whisper-based AI transcription services. (Specifically `.JSON` files following in the formats produced by: OpenAI Whisper, WhisperX, Faster-Whisper, Whisper.cpp. As well as `.dote` exports from [`MacWhisper`](https://goodsnooze.gumroad.com/l/macwhisper)).
...
5. Choose from the Whisper Import Options:
    ...
    - `Create Sync-codes from Timing Information`.
        ...
        - `Add silence markers for long pauses` - For pauses longer than 1 second, add a line showing the pause. For example: `(1.5)`
    - `Include Timestamps for each line` - Add a technical comment to each line showing the timing information provided for that line. For example: `// [Time: 0:22 - 00:27]`
    - If per-word translation confidence values are provided:
      - `Print average confidence % per line` - display this information as a technical comment at the end of each line. For example: `// [Avg. Confidence: 85%]`
    ...
    - `Remove Sentence Capitalisation` - from the body of the transcript to be imported. NOTE: this will also decapitalise proper nouns and abbreviations if they appear at the start of a sentence.
